...
class SignDetector:
	...
	pub_traffic_sign = rospy.Publisher('/detect/sign_number', UInt8, queue_size=1)
	pub_image_traffic_sign = rospy.Publisher('/detect/image_output/sign', Image, queue_size = 1)
	...
	def cbFindTrafficSign(self, image_msg):
		...
		if image_out_num == 1:
			...
			self.pub_image_traffic_sign.publish(self.cvBridge.cv2_to_imgmsg(cv_image_input, encoding="bgr8"))
		else:
			...
			# publishes traffic sign image in raw type
			self.pub_image_traffic_sign.publish(self.cvBridge.cv2_to_imgmsg(final, encoding="bgr8"))

	def main(self):
		rospy.spin()

traffic_signs = [Sign("park_yeri","park_yeri.png"),
				Sign("durak","durak.png"),]
				# Sign("park_etmek_yasaktir","park_etmek_yasaktir.png"),
				# Sign("giris_olmayan_yol","giris_olmayan_yol.png"),
